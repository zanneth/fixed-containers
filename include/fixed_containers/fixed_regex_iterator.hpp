...
public:
    ...
    {
        if (done_ || other.done_) return done_ == other.done_;
    ...
    [[nodiscard]] constexpr bool operator==(std::default_sentinel_t) const { return done_; }
...
public:
      ...
      : position_{first, last, regex, flags}
    {
        ...
        {
            if (count_ == MAXIMUM_SUBMATCHES) RegexType::fail(regex_constants::error_space);
            if (sub < -1) RegexType::fail(regex_constants::error_backref);
    ...
    {
        if (state_ == State::SUFFIX) return suffix_;
    ...
    [[nodiscard]] constexpr bool operator==(const FixedRegexTokenIterator& other) const
    {
        ...
        if (state_ == State::END) return true;
        if (state_ == State::SUFFIX) return suffix_ == other.suffix_;
    ...
    regex_constants::match_flag_type flags = regex_constants::match_default)
{
    ...
    {
        if (copy) output = std::copy(it->prefix().first, it->prefix().second, output);
        output = it->format(output, format, flags);
        tail = (*it)[0].second;
        if ((flags & regex_constants::format_first_only) != regex_constants::match_default) break;
    }
    if (copy) output = std::copy(tail, last, output);
    return output;
}
