...
// Included by fixed_regex.hpp after the expression type is complete.
    ...
    bool full) const
{
    ...
    if (!IMPLEMENTATION_DETAIL_DO_NOT_USE_valid_) return false;
    if (mark_count() > C) fail(regex_constants::error_space);
    ...
    struct Frame
    {
        FrameKind kind = FrameKind::CHOICE;
        std::size_t target = 0;
        It position{};
        Capture capture{};
        RepeatState repeat{};
    ...
    struct Machine
    {
        ...
        constexpr void tick()
        {
            if (operations == O) fail(regex_constants::error_complexity);
            ++operations;
        }
        ...
        {
            if (stack_size == W) fail(regex_constants::error_stack);
            stack[stack_size++] = frame;
        }
        constexpr void save_capture(std::size_t index, const Capture& value)
        {
            push(Frame{.kind = FrameKind::CAPTURE, .target = index, .capture = captures[index]});
            captures[index] = value;
        }
        constexpr void save_repeat(std::size_t index, RepeatState value)
        {
            push(Frame{.kind = FrameKind::REPEAT, .target = index, .repeat = repeats[index]});
            repeats[index] = value;
        }
        constexpr void undo(const Frame& frame)
        {
            if (frame.kind == FrameKind::CAPTURE) captures[frame.target] = frame.capture;
            if (frame.kind == FrameKind::REPEAT) repeats[frame.target] = frame.repeat;
        }
        constexpr bool backtrack()
        {
            while (stack_size != 0)
            {
                ...
                undo(frame);
                if (frame.kind == FrameKind::CHOICE)
                {
                    pc = frame.target;
                    current = frame.position;
                    return true;
                }
                if (frame.kind == FrameKind::SIMPLE_RETRY)
                {
                    auto retry = frame;
                    ...
                    if (instruction.negate)
                    {
                        ...
                        ++retry.repeat.count;
                        if (retry.repeat.count < instruction.maximum) push(retry);
                    }
                    else
                    {
                        --current;
                        retry.position = current;
                        if (current != frame.repeat.position) push(retry);
                    }
                    pc = instruction.left;
                    return true;
                }
                if (frame.kind == FrameKind::ASSERTION)
                {
                    current = frame.position;
        ...
        constexpr bool finish_assertion()
        {
            ...
            if (frame.negative)
            {
                while (stack_size > boundary + 1) undo(stack[--stack_size]);
                stack_size = boundary;
            }
            else
            {
                ...
                {
                    if (stack[read].kind == FrameKind::CAPTURE ||
                        stack[read].kind == FrameKind::REPEAT)
        ...
        [[nodiscard]] constexpr bool line_end(char ch) const { return ch == '\n' || ch == '\r'; }
        [[nodiscard]] constexpr bool matches_atom(const Node& atom) const
        {
            ...
            if (atom.op == Op::ANY) return regex.ecma() ? !line_end(*current) : *current != '\0';
        ...
        constexpr bool simple_repeat(const Node& instruction)
        {
            ...
            {
                tick();
                if (!matches_atom(atom)) return false;
                ++current;
                ++count;
            }
            ...
            if (instruction.negate ? count < instruction.maximum : count > instruction.minimum)
                push(Frame{.kind = FrameKind::SIMPLE_RETRY,
                           .target = pc,
                           .position = current,
        ...
        [[nodiscard]] constexpr bool at_begin() const
        {
            ...
            if (!regex.ecma() || !regex.has(multiline)) return false;
        ...
        {
            if (current == last) return !has_flag(regex_constants::match_not_eol);
            return regex.ecma() && regex.has(multiline) && line_end(*current);
        }
        [[nodiscard]] constexpr bool word(char ch) const
        {
            constexpr std::string_view NAME = "w";
            const auto& traits = regex.IMPLEMENTATION_DETAIL_DO_NOT_USE_traits_;
            return traits.isctype(ch, traits.lookup_classname(NAME.begin(), NAME.end(), false));
        }
        [[nodiscard]] constexpr bool boundary() const
        {
            if (current == first && !has_flag(regex_constants::match_prev_avail) &&
                ...
                return false;
            if (current == last && has_flag(regex_constants::match_not_eow)) return false;
        ...
        constexpr bool backreference(std::size_t group)
        {
            ...
            if (!capture.matched) return regex.ecma();
        ...
        {
            if (!found || length != best_length) return !found || length > best_length;
            // POSIX subexpression priority: earlier subexpressions take precedence.
            for (std::size_t i = 1; i <= regex.mark_count(); ++i)
            {
                if (captures[i].matched != best[i].matched) return captures[i].matched;
                const auto a = captures[i].length();
                const auto b = best[i].length();
                if (a != b) return a > b;
            }
            return false;
        }
        constexpr bool run(It candidate)
        {
            ...
            found = false;
            best_length = -1;
            for (auto& capture : captures) capture = {last, last, false};
            while (true)
            {
                tick();
                ...
                case Op::REPEAT_STEP:
                {
                    ...
                    if (success) save_repeat(index, {repeat.count + 1, repeat.position});
                    break;
                }
                case Op::REPEAT:
                {
                ...
                case Op::LOOK:
                {
                    ...
                    push(Frame{.kind = FrameKind::ASSERTION,
                ...
                case Op::ACCEPT:
                        ...
                        (!has_flag(regex_constants::match_not_null) || current != start))
                    {
                        ...
                        if (regex.ecma() || has_flag(regex_constants::match_any)) return true;
                    }
                    success = false;
                    break;
                case Op::EMPTY:
