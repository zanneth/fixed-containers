...
struct FixedSubMatch : std::pair<BidirectionalIt, BidirectionalIt>
{
    ...
    {
        string_type result{};
        if (matched) result.assign(this->first, this->second);
        return result;
    }
    ...
    [[nodiscard]] constexpr int compare_range(It begin, It end) const
    {
        ...
        {
            if (std::char_traits<char>::lt(*current, *begin)) return -1;
            if (std::char_traits<char>::lt(*begin, *current)) return 1;
        }
        ...
        regex_constants::match_flag_type flags = regex_constants::format_default) const
    {
        ...
        while (first != last)
        {
            ...
            if (sed)
            {
                ...
                else if (ch == '\\' && first != last)
                {
                    ch = *first++;
                    if (ch >= '0' && ch <= '9')
                        output = emit(output, (*this)[static_cast<size_type>(ch - '0')]);
                    else
                        *output++ = ch;
                }
                else
            ...
            else if (ch == '$' && first != last)
            {
                ...
                {
                    ++first;
                    auto index = static_cast<size_type>(ch - '0');
                ...
                else if (ch == '$' || ch == '&' || ch == '`' || ch == '\'')
                {
                    ...
                    if (ch == '$')
                        *output++ = '$';
                        ...
                        output = emit(output, ch == '`' ? prefix() : suffix());
                }
                else
                    *output++ = '$';
            }
