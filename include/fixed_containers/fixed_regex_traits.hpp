...
struct FixedRegexLocale
{
    ...
    constexpr FixedRegexLocale()
    {
        for (std::size_t i = 0; i < 256; ++i)
        {
            ...
            const bool space = ch == ' ' || (ch >= '\t' && ch <= '\r');
            const bool print = ch >= 32 && ch <= 126;
            const bool graph = ch >= 33 && ch <= 126;
            lowercase[i] = upper ? static_cast<unsigned char>(ch + ('a' - 'A')) : ch;
            collation[i] = ch;
            primary[i] = lowercase[i];
            classes[i] = static_cast<std::uint16_t>(
                (alpha || digit ? 1 : 0) | (alpha ? 2 : 0) | (ch == ' ' || ch == '\t' ? 4 : 0) |
                (ch < 32 || ch == 127 ? 8 : 0) | (digit ? 16 : 0) | (graph ? 32 : 0) |
                (lower ? 64 : 0) | (print ? 128 : 0) | (graph && !alpha && !digit ? 256 : 0) |
                (space ? 512 : 0) | (upper ? 1024 : 0) |
                (digit || (ch >= 'A' && ch <= 'F') || (ch >= 'a' && ch <= 'f') ? 2048 : 0) |
                (alpha || digit || ch == '_' ? 4096 : 0));
        }
    }
...
public:
    ...
    [[nodiscard]] constexpr char translate(char ch) const { return ch; }
    [[nodiscard]] constexpr char translate_nocase(char ch) const
    {
        return static_cast<char>(
            IMPLEMENTATION_DETAIL_DO_NOT_USE_locale_.lowercase[static_cast<unsigned char>(ch)]);
    }
                                                             ...
                                                             bool icase = false) const
    {
        ...
        for (; first != last; ++first)
        {
            if (name.size() == name.max_size()) return 0;
            const char ch = *first;
        ...
        for (std::size_t i = 0; i < NAMES.size(); ++i)
        {
            if (name == NAMES[i])
            {
                if (icase && (i == 6 || i == 10)) return 2;
                return static_cast<char_class_type>(1U << i);
            }
        }
        if (name == "d") return 16;
        if (name == "s") return 512;
        return 0;
    }
    template <class ForwardIt>
    [[nodiscard]] constexpr string_type lookup_collatename(ForwardIt first, ForwardIt last) const
    {
        ...
        for (; first != last; ++first)
        {
            if (name.size() == name.max_size()) return {};
        ...
        {
            if (name == CONTROL_NAMES[i]) return string_type(1, static_cast<char>(i));
        }
        ...
        {
            if (name == entry.first) return string_type(1, entry.second);
        }
        return {};
    }
    [[nodiscard]] constexpr bool isctype(char ch, char_class_type mask) const
    {
        return (IMPLEMENTATION_DETAIL_DO_NOT_USE_locale_.classes[static_cast<unsigned char>(ch)] &
                mask) != 0;
    }
    [[nodiscard]] constexpr int value(char ch, int radix) const
    {
        int result = -1;
        if (ch >= '0' && ch <= '9')
            result = ch - '0';
